...
import com.example.backend.servicios.MessageServicio;
...
/*ControlMessage es un REST Controler. Esta clase ControlMessage Gestionara las peticiones que se realicen
...
//Se debe crrear un metodo por cada paticion a travez de MessageServicio
...
public class ControlMessage {
    ...
    Con estas intruciones en el navegador podemos consultar por todos los Messages con :
    en el navegador ( :NumeroPuerto/api/Message/all ) con esta direccion estaria llamando a este metodo
    getMessages() */
    @GetMapping("/all")
    public List<Message> getMessages(){
        return messageServicio.getAll();
    }
    //Meetodo GET individual NO con el id sino directamente el numero. Ejemplo : /api/Message/8
    @GetMapping("/{id}")
    ...
    public Message save( @RequestBody Message comp){
        return messageServicio.save(comp);
    }
    @PutMapping("/update")
    @ResponseStatus(HttpStatus.CREATED)
    public Message update( @RequestBody Message comp){
        return messageServicio.update(comp);
    }
    @DeleteMapping("/{id}")
