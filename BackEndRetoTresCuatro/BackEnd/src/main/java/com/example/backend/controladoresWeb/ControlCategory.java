...
import com.example.backend.servicios.CategoryServicio;
...
/*ControlCategory es un REST Controler. Esta clase ControlCategory Gestionara las peticiones que se realicen
...
public class ControlCategory {
    ...
    Con estas intruciones en el navegador podemos consultar por todos los Categorys con :
    en el navegador ( :NumeroPuerto/api/Category/all ) con esta direccion estaria llamando a este metodo
    getCategorys() */
    @GetMapping("/all")
    public List<Category> getCategorys(){
        return CategoryServicio.getAll();
    }
    //Meetodo GET individual NO con el id sino directamente el numero. Ejemplo : /api/Category/8
    @GetMapping("/{id}")
    ...
    public Optional<Category> getCategory(@PathVariable ("id") int id ){
        return CategoryServicio.getCategoryId(id);
    }
    @PostMapping("/save")
    ...
    public Category save( @RequestBody Category comp){
        return CategoryServicio.save(comp);
    }
    @PutMapping("/update")
    @ResponseStatus(HttpStatus.CREATED)
    public Category update( @RequestBody Category comp){
        return CategoryServicio.update(comp);
    }
    @DeleteMapping("/{id}")
